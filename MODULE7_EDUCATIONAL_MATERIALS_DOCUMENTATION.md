...
**Note:** The uploaded file URL will be stored in `file_url` field. Files are uploaded directly to Supabase Storage bucket (if configured) or fallback to Django's default storage.
...
3. Create bucket in Supabase Storage dashboard
4. Files will be uploaded to: `articles/{article_id}/{uuid-filename.ext}`
